...
public class Acting {
	...
	public static final int SINGLE = 80000;
	public static final int DOUBLE = 100000;
...
// 방 예약과 예약취소
	public void reservation(String customerName, String customerPhoneNumber, int roomNum) {
		...
		for (int i = 0; i < rooms.length; i++) {
			if (rooms[i].getRoomNumber() == roomNum) {
				roomFound = true;
				if (rooms[i].getCustomerInformation() == null && rooms[i].getState() == EMPTY_ROOM) {
					...
					for (int j = 0; j < memberships.length; j++) {
						if (memberships[j] != null && memberships[j].getMembershipName().equals(customerName)
							...
							} else if (memberships[j].getGrade() == 1) {
								System.out.println("골드 회원입니다.(10%할인)");
							} else if (memberships[j].getGrade() == 2) {
								System.out.println("플래티넘 회원입니다.(20%할인)");
							} else {
					...
					for (int j = 0; j < memberships.length; j++) {
						if (memberships[j] != null && memberships[j].getMembershipName().equals(customerName)
								&& memberships[j].getMembershipPhoneNum().equals(customerPhoneNumber)) {
							if (foundMembership) {
								if (memberships[j].getGrade() == 1 && rooms[i].getRoomNumber() % 2 == 0) {
									m.setMoney(m.getMoney() + DOUBLE - 10000);
									System.out.println("90000원 결제 되었습니다.");
									System.out.println("예약이 완료되었습니다.");
								} else if (memberships[j].getGrade() == 2 && rooms[i].getRoomNumber() % 2 == 0) {
									m.setMoney(m.getMoney() + DOUBLE - 20000);
									System.out.println("80000원 결제 되었습니다.");
									System.out.println("예약이 완료되었습니다.");
								} else if (memberships[j].getGrade() == 1 && rooms[i].getRoomNumber() % 2 == 1) {
									m.setMoney(m.getMoney() + SINGLE - 8000);
									System.out.println("72000원 결제 되었습니다.");
									System.out.println("예약이 완료되었습니다.");
								} else if (memberships[j].getGrade() == 2 && rooms[i].getRoomNumber() % 2 == 0) {
									m.setMoney(m.getMoney() + SINGLE - 16000);
									System.out.println("64000원 결제 되었습니다.");
									System.out.println("예약이 완료되었습니다.");
								}
									...
									m.setMoney(m.getMoney() + DOUBLE);
									System.out.println("100000원 결제 되었습니다.");
									System.out.println("예약이 완료되었습니다.");

								} else if (rooms[i].getRoomNumber() % 2 == 1) {
									m.setMoney(m.getMoney() + SINGLE);
									System.out.println("80000원 결제 되었습니다.");
									System.out.println("예약이 완료되었습니다.");
		...
		for (int i = 0; i < rooms.length; i++) {
			...
			if (rooms[i].getRoomNumber() == originRoomNum && (rooms[i].getState() == 1 || rooms[i].getState() == 15
				...
				for (int j = 0; j < rooms.length; j++) {
					if (rooms[j].getRoomNumber() == transferRoomNum && rooms[j].getState() == EMPTY_ROOM) {
	...
	public void walkIn(int roomNum) {
		for (int i = 0; i < rooms.length; i++) {
			if (rooms[i].getRoomNumber() == roomNum) {
				if (rooms[i].getState() == EMPTY_ROOM) {
					rooms[i].setState(2);
